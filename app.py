...
def process_document(uploaded_file: UploadedFile) -> list[Document]:
    ...
    if not splits:
        st.error("Failed to split documents. Please check the input file.")
        return
...
def get_vector_collection() -> chromadb.Collection:
    ...
    # Debugging
    st.write("Embedding function initialized successfully.")
    return collection
...
def add_to_vector_collection(all_splits: list[Document], file_name: str):
    ...
    if not documents:
        st.error("No valid documents found to add to the vector store.")
        return
    else:
        st.write(f"Number of documents to add: {len(documents)}")
    ...
    try:
        ...
        st.error(f"Error while upserting: {str(e)}")
        raise
